...
Lab8::Lab8()
{
    lightPosition2 = glm::vec3(-2, 2, 0);
    lightDirection2 = glm::vec3(1, -1, 0);
    lightColor2 = glm::vec3(0, 0.5f, 1.0f);

    useSpotlight = 0;
    spotCutoff = cos(glm::radians(30.0f));
    spotOuterCutoff = cos(glm::radians(45.0f));
}
...
void Lab8::Update(float deltaTimeSeconds)
{
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.8f, 0.8f, 0.8f));
...
void Lab8::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
    ...
    int light_position2_loc = glGetUniformLocation(shader->program, "light_position2");
    glUniform3f(light_position2_loc, lightPosition2.x, lightPosition2.y, lightPosition2.z);

    int light_direction2_loc = glGetUniformLocation(shader->program, "light_direction2");
    glUniform3f(light_direction2_loc, lightDirection2.x, lightDirection2.y, lightDirection2.z);

    int light_color2_loc = glGetUniformLocation(shader->program, "light_color2");
    glUniform3f(light_color2_loc, lightColor2.r, lightColor2.g, lightColor2.b);

    int use_spot_loc = glGetUniformLocation(shader->program, "use_spot");
    glUniform1i(use_spot_loc, useSpotlight);

    int spot_cutoff_loc = glGetUniformLocation(shader->program, "spot_cutoff");
    glUniform1f(spot_cutoff_loc, spotCutoff);

    int spot_outer_cutoff_loc = glGetUniformLocation(shader->program, "spot_outer_cutoff");
    glUniform1f(spot_outer_cutoff_loc, spotOuterCutoff);
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F) {
        useSpotlight = 1 - useSpotlight;
        cout << "Spotlight Mode: " << (useSpotlight ? "ON" : "OFF") << endl;
    }
